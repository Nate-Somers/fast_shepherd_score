...
| **Screen fast engine (step 10)** | `shepherd_score/screen.py` | Needed **before you report screening throughput** (the object path's per-shard `MoleculePair` rebuild understates aligns/s vs the resident-tensor fast modes; identical GPU kernel). Six edits: `_FAST_MODES` + `_FastPair.__slots__` (result + new tensor slots) + `_query_ref_arrays` + `_ref_tensors_from_arrays` + `_build_fit_fast_pairs` + `_fast_batch_kwargs`. Pre-set every tensor the driver's `_batch_upload` would fill so it skips the `.ref_molec`/`.fit_molec` lambdas. `vol_tversky` = drop-in copy of `vol`; `esp_field` adds four field-point tensors. |
