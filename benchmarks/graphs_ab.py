...
import shepherd_score.alignment.utils.fast_esp_se3 as fesp
...
def _set_graphs(mode, on):
    (fesp if mode == "esp" else fse3)._FINE_GRAPHS = on


def _time(mode, pairs, steps, reps):
    _align(mode, pairs, steps); torch.cuda.synchronize()  # warmup (captures graph if on)
    ts = []
    for _ in range(reps):
...
def main():
    ...
    ap.add_argument("--mode", default="surf", choices=["surf", "esp"])
    ...
    print(f"CUDA-GRAPH end-to-end A/B  align_batch_{args.mode}  off vs on  (MIN/{args.reps} reps, "
    ...
    for nb in args.batches:
        co = make_real_cohort(args.mode, n_pairs=nb, bucket_kind="same", seed=3)
        pairs = [MoleculePair(p.ref, p.fit, do_center=False, device=dev) for p in co.pairs]
        _set_graphs(args.mode, False)
        t_off, s_off = _time(args.mode, pairs, args.steps, args.reps)
        _set_graphs(args.mode, True)
        t_on, s_on = _time(args.mode, pairs, args.steps, args.reps)
