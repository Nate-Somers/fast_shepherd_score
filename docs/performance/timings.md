...
Sharding a 10^6-conformer `vol` screen across L40S devices with `screen(ndev=k)`, one worker process per device kept for the life of the calling process, gave 1,075,486 alignments/s on one device, 2,133,918 on two (2.0x) and 3,379,473 on four (3.1x): 0.93, 0.47 and 0.30 s per screen after the first. Starting the workers costs 2.9 s (two devices) or 4.0 s (four), once per process, and each screen then carries about 0.01 s of fixed cost. On CPU the fork-sharded driver (`shepherd_score.accel.screen_parallel`), with its pool kept across calls, each worker pinned to its own physical core and the library dealt out strided, gave 1,854 alignments/s on one worker, 14,886 on 8 (8.0x), 29,003 on 16 (15.6x), 56,285 on 32 (30.4x) and 100,019 on 64 (54x) for N = 10^4 read from the same store on an exclusively allocated 96-core node; its one-worker rate is 0.80x the in-process screen of that store (2,304 alignments/s). Before b3599dc the multi-device workers ran per-molecule seeds on a canonical store (two devices no faster than one) and unpinned workers shared hyperthread siblings (64 workers 35x); before f43a1bd both drivers restarted their workers on every call (four devices no faster than one, 64 workers 4.4x at N = 10^5). Both are documented in the paper repository's results.
