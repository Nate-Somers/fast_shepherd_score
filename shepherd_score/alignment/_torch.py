...
def _esp_field_overlap(ref_centers: torch.Tensor,
                       fit_centers: torch.Tensor,
                       ...
                       alpha_field: float,
                       lam: float,
                       field_weight: float) -> torch.Tensor:
    """Combined shape + ESP field-point similarity (helper shared by the objective/optimizer).

    ``shape`` is the atom-centred Gaussian volume Tanimoto; ``field`` is the ESP overlap of the
    signed field points (sign used as the "charge"). If either point set is empty the field
    channel is 0 (shape-only). ``fit_*`` are assumed already SE(3)-transformed. Returns the
    combined similarity with shape matching ``get_overlap(ref_centers, fit_centers)``.
    """
    shape_sim = get_overlap(ref_centers, fit_centers, alpha)
    n_ref_fp = ref_fp_pos.shape[-2]
    n_fit_fp = fit_fp_pos.shape[-2]
    if n_ref_fp == 0 or n_fit_fp == 0:
        field_sim = torch.zeros_like(shape_sim)
    else:
        field_sim = get_overlap_esp(centers_1=ref_fp_pos,
                                    centers_2=fit_fp_pos,
                                    charges_1=ref_fp_sign,
                                    charges_2=fit_fp_sign,
                                    alpha=alpha_field,
                                    lam=lam).reshape(shape_sim.shape)
    return (1 - field_weight) * shape_sim + field_weight * field_sim


def objective_esp_field_overlay(se3_params: torch.Tensor,
                                ref_centers: torch.Tensor,
                                fit_centers: torch.Tensor,
                                ref_fp_pos: torch.Tensor,
                                fit_fp_pos: torch.Tensor,
    ...
    """
    Objective for the Cresset-style ``esp_field`` overlay: a weighted combination of atom-centred
    Gaussian *shape* (volume) Tanimoto and the *ESP field-point* Tanimoto (overlap of the signed
    extrema of the softened molecular electrostatic potential, matched by sign).

    The combined similarity is ``(1 - field_weight) * shape + field_weight * field``. Only the fit
    inputs are transformed; both the fit atom centres AND the fit field points move rigidly under
    the same SE(3) pose. If either molecule has no field points the field channel is 0 (shape-only).
    Supports batched and non-batched inputs; for a batch the loss is the average.
    ...
    ref_centers, fit_centers : torch.Tensor (N,3)/(M,3) or batched (B,N,3)/(B,M,3)
        Heavy-atom coordinates for the Gaussian shape overlap.
    ref_fp_pos, fit_fp_pos : torch.Tensor (P,3)/(Q,3) or batched
        ESP field-point coordinates (may be empty, P=0 or Q=0).
    ref_fp_sign, fit_fp_sign : torch.Tensor (P,)/(Q,) or batched
        Field-point signs (+1 = potential maximum, -1 = minimum), used as the ESP "charge".
    alpha : float
        Gaussian width for the shape overlap (0.81 = volumetric).
    alpha_field : float
        Gaussian width for the field-point positional overlap.
    lam : float
        Sign ("charge") weighting for the field-point ESP overlap (0.1 = atom-centred convention).
    field_weight : float
        Weight of the field channel in [0, 1]; shape gets ``1 - field_weight``.
    ...
    se3_matrix = get_SE3_transform(se3_params)
    fit_centers = apply_SE3_transform(fit_centers, se3_matrix)
    if fit_fp_pos.shape[-2] != 0:
        fit_fp_pos = apply_SE3_transform(fit_fp_pos, se3_matrix)

    combo = _esp_field_overlap(ref_centers, fit_centers,
                               ref_fp_pos, fit_fp_pos,
                               ref_fp_sign, fit_fp_sign,
                               alpha, alpha_field, lam, field_weight)
...
def optimize_esp_field_overlay(ref_centers: torch.Tensor,
                               fit_centers: torch.Tensor,
                               ref_fp_pos: torch.Tensor,
                               fit_fp_pos: torch.Tensor,
                               ref_fp_sign: torch.Tensor,
    ...
    """
    Optimize a Cresset-style combined shape (atom-centred Gaussian volume) + ESP field-point
    overlay over SE(3), maximizing
    ``(1 - field_weight) * shape_Tanimoto + field_weight * field_Tanimoto``.
    ...
    the atom clouds (``ref_centers``/``fit_centers``), matching the ``vol``/``vol_color`` optimizers.
    Both the fit atom centres and the fit field points are transformed by the pose. Self-overlaps
    are recomputed every step (both channels self-normalise), so a self-copy scores 1.000 when it
    has field points and ``1 - field_weight`` (shape only) when it has none.

    Parameters
    ----------
    ref_centers, fit_centers : torch.Tensor (N,3) / (M,3)
        Heavy-atom coordinates for the Gaussian shape overlap.
    ref_fp_pos, fit_fp_pos : torch.Tensor (P,3) / (Q,3)
        ESP field-point coordinates (may be empty, P=0 or Q=0).
    ref_fp_sign, fit_fp_sign : torch.Tensor (P,) / (Q,)
        Field-point signs (+1 max, -1 min), used as the ESP "charge".
    alpha : float (default=0.81)
        Gaussian width for the shape overlap.
    alpha_field : float (default=0.81)
        Gaussian width for the field-point positional overlap.
    lam : float (default=0.1)
        Sign weighting for the field-point ESP overlap.
    field_weight : float (default=0.5)
        Weight of the field channel in [0, 1].
    num_repeats : int (default=50)
        Number of SE(3) initializations.
    trans_centers : torch.Tensor (T,3) or None
        Optional translation seed centers (10 rotations per center); otherwise COM-aligned.
    ...
    SE3_transform : torch.Tensor (4,4)
        Best SE(3) transformation.
    score : torch.Tensor
        Best combined (shape + field) similarity.
    """
    # Seed SE(3) from the shape point clouds (consistent with the vol/vol_color optimizers).
    if trans_centers is None:
        se3_params = _initialize_se3_params(ref_points=ref_centers, fit_points=fit_centers, num_repeats=num_repeats)
    else:
        se3_params = _initialize_se3_params_with_translations(
    ...
    has_fp = ref_fp_pos.shape[-2] != 0 and fit_fp_pos.shape[-2] != 0

    # Repeat per-molecule tensors across the seed batch (squeeze(0) -> single instance).
    ref_centers_rep = ref_centers.repeat((num_repeats, 1, 1)).squeeze(0)
    fit_centers_rep = fit_centers.repeat((num_repeats, 1, 1)).squeeze(0)
    if has_fp:
        ref_fp_pos_rep = ref_fp_pos.repeat((num_repeats, 1, 1)).squeeze(0)
        fit_fp_pos_rep = fit_fp_pos.repeat((num_repeats, 1, 1)).squeeze(0)
        ref_fp_sign_rep = ref_fp_sign.repeat((num_repeats, 1)).squeeze(0)
        fit_fp_sign_rep = fit_fp_sign.repeat((num_repeats, 1)).squeeze(0)
    else:
        # Empty point sets: keep an (…,0,3)/(…,0) shape so the objective's guard triggers.
        ref_fp_pos_rep, fit_fp_pos_rep = ref_fp_pos, fit_fp_pos
        ref_fp_sign_rep, fit_fp_sign_rep = ref_fp_sign, fit_fp_sign

    if verbose:
        init_score = _esp_field_overlap(ref_centers, fit_centers, ref_fp_pos, fit_fp_pos,
                                        ref_fp_sign, fit_fp_sign, alpha, alpha_field, lam, field_weight)
        print(f'Initial esp_field similarity score: {float(init_score):.3f}')

    last_loss = 1
    counter = 0
    for step in range(max_num_steps):
        loss = objective_esp_field_overlay(
            se3_params=se3_params,
            ref_centers=ref_centers_rep,
            fit_centers=fit_centers_rep,
            ref_fp_pos=ref_fp_pos_rep,
            fit_fp_pos=fit_fp_pos_rep,
            ref_fp_sign=ref_fp_sign_rep,
            fit_fp_sign=fit_fp_sign_rep,
            alpha=alpha,
    ...
    aligned_fit_centers = apply_SE3_transform(fit_centers_rep, SE3_transform)
    if has_fp:
        aligned_fit_fp_pos = apply_SE3_transform(fit_fp_pos_rep, SE3_transform)
    else:
        aligned_fit_fp_pos = fit_fp_pos_rep

    scores = _esp_field_overlap(ref_centers_rep, aligned_fit_centers,
                                ref_fp_pos_rep, aligned_fit_fp_pos,
                                ref_fp_sign_rep, fit_fp_sign_rep,
                                alpha, alpha_field, lam, field_weight)

    if num_repeats == 1:
        if verbose:
            print(f'Optimized esp_field similarity score: {float(scores):.3f}')
        best_alignment = aligned_fit_centers.cpu()
        best_transform = SE3_transform.cpu()
        best_score = scores.detach().cpu()
    else:
        if verbose:
            print(f'Optimized esp_field similarity -- max: {scores.max():.3f} | mean: {scores.mean():.3f} | min: {scores.min():.3f}')
        best_idx = torch.argmax(scores.detach().cpu())
        best_alignment = aligned_fit_centers.cpu()[best_idx]
        best_transform = SE3_transform.cpu()[best_idx]
        best_score = scores.detach().cpu()[best_idx]
...
def _vol_lipo_overlap(ref_centers: torch.Tensor,
                      fit_centers: torch.Tensor,
    ...
    n_fit = fit_lipo_pos.shape[-2]
    if n_ref == 0 or n_fit == 0:
        lipo_sim = torch.zeros_like(shape_sim)
    else:
        lipo_sim = get_overlap_esp(centers_1=ref_lipo_pos,
                                   centers_2=fit_lipo_pos,
                                   charges_1=ref_lipo,
                                   charges_2=fit_lipo,
                                   alpha=alpha,
                                   lam=lam).reshape(shape_sim.shape)
    return (1 - lipo_weight) * shape_sim + lipo_weight * lipo_sim
...
def objective_vol_lipo_overlay(se3_params: torch.Tensor,
    ...
    *shape* (volume) Tanimoto and *lipophilicity* Tanimoto (Crippen atomic logP overlaid like an
    ...
    combo = _vol_lipo_overlap(ref_centers, fit_centers,
                              ref_lipo_pos, fit_lipo_pos,
                              ref_lipo, fit_lipo,
                              alpha, lam, lipo_weight)
...
def optimize_vol_lipo_overlay(ref_centers: torch.Tensor,
    ...
    """
    Optimize a combined shape (atom-centred Gaussian volume) + lipophilicity (Crippen logP)
    overlay over SE(3), maximizing
    ...
    -------
    aligned_fit_centers : torch.Tensor (M,3)
        Transformed fit shape centres under the best SE(3).
    SE3_transform : torch.Tensor (4,4)
        Best SE(3) transformation.
    score : torch.Tensor
    ...
    if verbose:
        init_score = _vol_lipo_overlap(ref_centers, fit_centers, ref_lipo_pos, fit_lipo_pos,
                                       ref_lipo, fit_lipo, alpha, lam, lipo_weight)
    ...
    for step in range(max_num_steps):
        loss = objective_vol_lipo_overlay(
            ...
            ref_centers=ref_centers_rep,
            fit_centers=fit_centers_rep,
            ref_lipo_pos=ref_lipo_pos_rep,
            fit_lipo_pos=fit_lipo_pos_rep,
    ...
    scores = _vol_lipo_overlap(ref_centers_rep, aligned_fit_centers,
                               ref_lipo_pos_rep, aligned_fit_lipo_pos,
                               ref_lipo_rep, fit_lipo_rep,
                               alpha, lam, lipo_weight)
...
def objective_vol_tversky_overlay(se3_params: torch.Tensor,
                                  ref_points: torch.Tensor,
                                  fit_points: torch.Tensor,
                                  alpha: float = 0.81,
    ...
    """
    Objective for the asymmetric "fits-inside" ``vol_tversky`` shape overlay.

    Identical atom-centred Gaussian *shape* channel as ``objective_ROCS_overlay`` (only the
    fit is transformed), but the similarity *reduction* is Tversky rather than Tanimoto::

        T = AB / (AB + tversky_alpha * (AA - AB) + tversky_beta * (BB - AB))

    where ``AB`` is the cross overlap of the reference with the SE(3)-transformed fit, ``AA`` is
    the reference self-overlap, and ``BB`` is the fit self-overlap (all raw ``VAB_2nd_order``
    integrals). ``AA - AB`` is reference volume left uncovered ("missing" query volume) and
    ``BB - AB`` is extra fit volume. With the defaults (``tversky_alpha=0.95``,
    ``tversky_beta=0.05``) missing reference volume is penalized heavily while extra fit volume is
    barely penalized, so the score rewards the *reference* being contained in the fit
    ...
    alpha : float (default=0.81)
        Gaussian width for the shape overlap (0.81 = volumetric, heavy atoms).
    ...
    precomputed_AA, precomputed_BB : torch.Tensor, optional
        Pose-invariant reference / fit self-overlaps. Computed inside when ``None``.
    ...
    AB = VAB_2nd_order(ref_points, fit_points, alpha)
    AA = precomputed_AA if precomputed_AA is not None else VAB_2nd_order(ref_points, ref_points, alpha)
    BB = precomputed_BB if precomputed_BB is not None else VAB_2nd_order(fit_points, fit_points, alpha)
...
def optimize_vol_tversky_overlay(ref_points: torch.Tensor,
                                 fit_points: torch.Tensor,
                                 alpha: float = 0.81,
                                 tversky_alpha: float = 0.95,
                                 tversky_beta: float = 0.05,
    ...
    """
    Optimize the asymmetric "fits-inside" ``vol_tversky`` shape overlay over SE(3), maximizing
    the Tversky shape similarity
    ``AB / (AB + tversky_alpha * (AA - AB) + tversky_beta * (BB - AB))``.

    Same atom-centred Gaussian shape machinery and multi-start scheme (identity + 4
    principal-component + Fibonacci rotations, all COM-aligned) with Adam as the ``vol``/ROCS
    optimizer, but scored with an asymmetric Tversky reduction instead of Tanimoto. With the
    defaults the objective rewards the *reference* being contained in the fit. The pose-invariant
    self-overlaps ``AA``/``BB`` are precomputed once before the loop.
    ...
    # Pre-compute SE(3)-invariant self-overlaps once before the loop.
    with torch.no_grad():
        _AA = VAB_2nd_order(ref_points, ref_points, alpha)
        _BB = VAB_2nd_order(fit_points, fit_points, alpha)

    if verbose:
        with torch.no_grad():
            _init_AB = VAB_2nd_order(ref_points, fit_points, alpha)
            _init = _init_AB / (_init_AB + tversky_alpha * (_AA - _init_AB) + tversky_beta * (_BB - _init_AB))
        print(f'Initial vol_tversky similarity score: {float(_init):.3f}')

    last_loss = 1
    counter = 0
    for step in range(max_num_steps):
        loss = objective_vol_tversky_overlay(se3_params=se3_params,
                                             ref_points=ref_points,
                                             fit_points=fit_points_to_transform,
                                             alpha=alpha,
    ...
    AB = VAB_2nd_order(ref_points, aligned_points, alpha)
    scores = AB / (AB + tversky_alpha * (_AA - AB) + tversky_beta * (_BB - AB))

    if num_repeats == 1:
        if verbose:
            print(f'Optimized vol_tversky similarity score: {float(scores):.3f}')
        best_alignment = aligned_points.cpu()
        best_transform = SE3_transform.cpu()
        best_score = scores.detach().cpu()
    else:
        if verbose:
            print(f'Optimized vol_tversky similarity -- max: {scores.max():.3f} | mean: {scores.mean():.3f} | min: {scores.min():.3f}')
        best_idx = torch.argmax(scores.detach().cpu())
        best_alignment = aligned_points.cpu()[best_idx]
        best_transform = SE3_transform.cpu()[best_idx]
        best_score = scores.detach().cpu()[best_idx]
