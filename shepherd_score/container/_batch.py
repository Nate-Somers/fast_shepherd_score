...
class MoleculePairBatch:
    ...
    def align_with_esp_field(self,
        ...
        """Align all pairs using the Cresset-style ``esp_field`` overlay: atom-centred Gaussian
        *shape* (volume) + signed ESP *field-point* overlap, blended
        ``(1-field_weight)*shape_Tanimoto + field_weight*field_Tanimoto``.

        Both the heavy-atom centres AND the ESP field points of the fit move rigidly under the
        same SE(3) pose, and BOTH channels steer the pose (joint weighted gradient). If a pair
        has no field points on either side, its field channel is 0 (shape-only).

        Results are stored in-place on each MoleculePair:
        - ``pair.transform_esp_field`` and ``pair.sim_aligned_esp_field``
        ...
        alpha : float
            Gaussian width for the shape overlap (default 0.81, volumetric heavy atoms).
        alpha_field : float
            Gaussian width for the field-point positional overlap (default 0.81).
        lam : float
            Sign ("charge") weighting for the field-point ESP overlap (default 0.1, raw).
        num_repeats, lr, max_num_steps, verbose
            Standard optimization controls. ``num_repeats`` / ``max_num_steps`` default
            (``None``) to the per-mode ``MODE_SEEDS`` / ``MODE_STEPS`` in ``accel/_modes.py``.
        backend : str
            ``None`` (default) resolves device-aware (Triton on CUDA else numba). ``"triton"``
            (aliases ``"cuda"``/``"gpu"``) and ``"numba"`` (alias ``"cpu"``) route to the batched
            ``MoleculePair._align_batch_esp_field`` driver, which reuses the shape kernel (atom
            channel) and the fused ESP kernel (field-point channel). ``"jax"`` falls back to the
        ...
        """
        if max_num_steps is None:
            max_num_steps = _default_steps("esp_field")
        if num_repeats is None:
            num_repeats = _default_seeds("esp_field")
        handled, _result = self._run_fast_or_fallthrough(
            backend, MoleculePair._align_batch_esp_field,
            dict(field_weight=field_weight, alpha=alpha, alpha_field=alpha_field, lam=lam,
                 num_repeats=num_repeats, lr=lr, steps_fine=max_num_steps),
            "sim_aligned_esp_field", "transform_esp_field", "_fit_xyz_t",
            return_aligned)
        if handled:
            return _result

        return self._delegate_alignment(
            'align_with_esp_field', 'sim_aligned_esp_field',
